...
function PdfBlockFforLackOfMotivation({ logo, name, onDelete }) {
    ...
    useEffect(() => {
        ...
        const pdfsListRef = ref(storage, "LACKOFMOTIVATION/");
    ...
    const deletePdf = (url, name) => {
        ...
        if (confirmDelete) {
            const fileName = decodeURIComponent(url).split("?")[0].split("LACKOFMOTIVATION/")[1];
            const pdfRef = ref(storage, `LACKOFMOTIVATION/${fileName}`);
            deleteObject(pdfRef)
                .then(() => {
...
export default PdfBlockFforLackOfMotivation