...
function PdfBlockFforLimitingAcademicStress({ logo, name, onDelete }) {
    ...
    useEffect(() => {
        ...
        const pdfsListRef = ref(storage, "ACADEMICSTRESS/");
    ...
    const deletePdf = (url, name) => {
        ...
        if (confirmDelete) {
            const fileName = decodeURIComponent(url).split("?")[0].split("ACADEMIC STRESS/")[1];
            const pdfRef = ref(storage, `ACADEMIC STRESS/${fileName}`);
            deleteObject(pdfRef)
                .then(() => {
...
export default PdfBlockFforLimitingAcademicStress