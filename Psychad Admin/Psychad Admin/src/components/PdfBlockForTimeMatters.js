...
const PdfBlock = ({ logo, name, onDelete }) => {
  ...
  useEffect(() => {
    ...
    const pdfsListRef = ref(storage, "TIME_MATTERS/");
  ...
  const deletePdf = (url, name) => {
    ...
    if (confirmDelete) {
      const fileName = decodeURIComponent(url).split("?")[0].split("TIME_MATTERS/")[1];
      const pdfRef = ref(storage, `TIME_MATTERS/${fileName}`);
      deleteObject(pdfRef)
        .then(() => {
