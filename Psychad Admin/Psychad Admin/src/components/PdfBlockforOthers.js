...
function PdfBlockforOthers({ logo, name, onDelete }) {
  ...
  useEffect(() => {
    ...
    const pdfsListRef = ref(storage, "OTHERS/");
  ...
  const deletePdf = (url, name) => {
    ...
    if (confirmDelete) {
      const fileName = decodeURIComponent(url).split("?")[0].split("OTHERS/")[1];
      const pdfRef = ref(storage, `OTHERS/${fileName}`);
      deleteObject(pdfRef)
        .then(() => {
...
export default PdfBlockforOthers;