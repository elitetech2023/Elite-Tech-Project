...
function PdfBlockForStrengthAndTraining({ logo, name, onDelete }) {
    ...
    useEffect(() => {
        ...
        const pdfsListRef = ref(storage, "STRENGHTANDTRAINING/");
    ...
    const deletePdf = (url, name) => {
        ...
        if (confirmDelete) {
            const fileName = decodeURIComponent(url).split("?")[0].split("STRENGHTANDTRAINING/")[1];
            const pdfRef = ref(storage, `STRENGHTANDTRAINING/${fileName}`);
            deleteObject(pdfRef)
                .then(() => {
...
export default PdfBlockForStrengthAndTraining