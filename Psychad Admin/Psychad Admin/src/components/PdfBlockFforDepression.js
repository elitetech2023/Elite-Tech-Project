...
function PdfBlockFforDepression({ logo, name, onDelete }) {
  ...
  useEffect(() => {
    ...
    const pdfsListRef = ref(storage, "DEPRESSION/");
  ...
  const deletePdf = (url, name) => {
    ...
    if (confirmDelete) {
      const fileName = decodeURIComponent(url).split("?")[0].split("DEPRESSION/")[1];
      const pdfRef = ref(storage, `DEPRESSION/${fileName}`);
      deleteObject(pdfRef)
        .then(() => {
...
export default PdfBlockFforDepression